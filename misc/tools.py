...
def semester_anlegen(x_updated, df, personen_uebernehmen, anforderung_uebernehmen, veranstaltung_uebernehmen):
    ...
    _doc.clear()
    util.list_semesters.clear()
    flash("Semester erfolgreich angelegt!")
...
def delete_semester(id):
    ...
    _doc.clear()
    util.list_semesters.clear()
    flash("🎉 Semester gelöscht, alle Personen, Studiengänge und Veranstaltungen geupdated.")
...
def display_navigation():
    ...
    with st.sidebar:
        ...
        # format_func-Mapping vorberechnen, damit die Lambda kein find_one mehr macht.
        semesters = util.list_semesters()
        sem_label = {s["_id"]: f"{s['name_de']} {'😎' if s['hp_sichtbar'] else ''}" for s in semesters}
        sem_ids = [s["_id"] for s in semesters]
        st.session_state.semester_id = st.selectbox(label="Semester", options = sem_ids, index = sem_ids.index(st.session_state.semester_id), format_func = (lambda a: sem_label[a]), placeholder = "Wähle ein Semester", label_visibility = "collapsed", key = "master_semester_choice")
