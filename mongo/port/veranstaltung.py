...
def port(mongo_db, files):
    ...
    for file in files:
        ...
        table_name = "Course"
        trans = {'id': 'kurzname', 'notes': 'kommentar_latex_de'}
        skip = ['lsfid']
        logging.info("Entering " + table_name)
        df = pd.read_sql_query(f"SELECT * from {table_name}", conn)
        df.rename(columns = trans, inplace = True)
        df["href"] = [x if x is not None else "" for x in df["href"]]        
        df["url"] = [x if str.split(x, ":")[0] == "https" else "" for x in df["href"]]        
...
#        df['id'] = [semester_shortname + "-" + x.lower() for x in df['shortname']]
        ...
        for x in posts:
            ...
            x["vorkenntnisse_de"] = ""
            x["vorkenntnisse_en"] = ""
            x["kommentar_latex_de"] = ""
            x["kommentar_latex_en"] = ""
            x["verwendbarkeit_modul"] = []
            x["verwendbarkeit_anforderung"] = []
            x["verwendbarkeit"] = []
            x["dozent"] = []
            x["assistent"] = []
            x["organisation"] = []
            x["woechentlicher_termin"] = []
            x["einmaliger_termin"] = []
            x["kommentar_html_de"] = ""
            x["kommentar_html_en"] = ""
            cou.insert_one(x)
        ...
        for item, row in df_lecturer.iterrows():
            ...
            c = cou.find_one_and_update({'kurzname': row['course_id'], "semester": sem_id}, {'$push': {'dozent': my_person}})
            per.update_one({'_id': my_person}, {'$push': {'veranstaltung': c["_id"]}})
        ...
        for item, row in df_cou_req.iterrows():
            ...
            if my_requirement:                 
                ...
                cou.update_one({'_id': my_course}, {'$push': {'verwendbarkeit_modul': my_module, 'verwendbarkeit_anforderung': m, 'verwendbarkeit': verwendbarkeit}})
        # Lösche Kurse ohne Kategorie
        cou.delete_many({"kategorie": ""})            
        # Trage leere URL ein, falls nötig:
        for c in list(cou.find({})):
            c["url"] = ""
