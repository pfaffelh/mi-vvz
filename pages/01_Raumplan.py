...
#    st.session_state.semester = sem_id
    if st.session_state.semester_id is not None:
        ...
        st.session_state.hauptraum_ids = st.multiselect("Räume", raum_dict.keys(), st.session_state.hauptraum_ids, format_func = (lambda a: raum_dict[a]), placeholder = "Bitte auswählen", help = tools.hilfe("raumplan.raeume"))
        st.write("<hr style='height:1px;margin:0px;border:none;color:#333;background-color:#333;' /> ", unsafe_allow_html=True)
