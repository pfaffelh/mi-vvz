...
stud = list(util.studiengang.find({"sichtbar": True}, sort = [("kurzname", pymongo.DESCENDING)]))
stu_dict = {}
for s in stud:
...
if st.session_state.logged_in:
    ...
    if st.session_state.edit == "" or st.session_state.page != "Modul":
        ...
        with st.form(f'ID-{x["_id"]}'):
            ...
            stu_list = st.multiselect("Studiengänge", [x["_id"] for x in util.studiengang.find({"$or": [{"sichtbar": True}, {"_id": {"$in": x["studiengang"]}}]}, sort = [("rang", pymongo.ASCENDING)])], x["studiengang"], format_func = (lambda a: repr(util.studiengang, a, False)), placeholder = "Bitte auswählen")
